...
export const locales: Record<Lang, { hreflang: string; og: string; path: string; label: string; dir: 'ltr' | 'rtl' }> = {
...
export const seoByLang: Record<Lang, {
  title: string
  description: string
  h1: string
  lead: string
...
export function getSiteUrl() {
  return (import.meta.env.VITE_SITE_URL || SITE_URL).replace(/\/$/, '')
}

export function getCanonicalUrl(lang: Lang, baseUrl = getSiteUrl()) {
  return `${baseUrl}${locales[lang].path}`
}

export function getLangFromPath(pathname: string): Lang {
  const segment = pathname.split('/').filter(Boolean)[0]
  return supportedLangs.includes(segment as Lang) ? (segment as Lang) : 'pt'
}

export function getAlternateLinks(lang: Lang, baseUrl = getSiteUrl()) {
  void lang
  return supportedLangs.map((code) => ({
    lang: locales[code].hreflang,
    href: getCanonicalUrl(code, baseUrl),
  }))
}

export function getJsonLd(lang: Lang, baseUrl = getSiteUrl()) {
  const seo = seoByLang[lang]
  const url = getCanonicalUrl(lang, baseUrl)
  ...
  return [
    {
      ...
      contactPoint: {
        ...
        availableLanguage: ['Portuguese', 'English', 'Spanish', 'Arabic'],
      },
    },
    {
      '@context': 'https://schema.org',
    ...
    },
    {
      ...
      primaryImageOfPage: `${baseUrl}${DEFAULT_IMAGE}`,
      breadcrumb: { '@id': `${url}#breadcrumb` },
      alternateName: seo.h1,
      potentialAction: { '@type': 'CommunicateAction', target: 'https://wa.me/5512988051401' },
    },
    {
      '@context': 'https://schema.org',
      '@type': 'Service',
      ...
      itemListElement: [
        {
          '@type': 'ListItem',
          position: 1,
          name: lang === 'en' ? 'Home' : lang === 'es' ? 'Inicio' : lang === 'ar' ? 'الرئيسية' : 'Inicio',
          item: url,
        },
      ],
    },
